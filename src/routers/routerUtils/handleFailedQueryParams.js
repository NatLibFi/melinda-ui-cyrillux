...
export function handleFailedQueryParams(req, res, next) {
  ...
  const failedParams = [...checkCyrilluxQueryParams(queryParams)]
    .filter(param => !param.value)
    .map(param => param.name);

  if (failedParams.length === 0) {
    logger.debug(`Query parameter check for app passed!`);
    logger.debug('.-._.-._.-._.-._.-._.-._.-._.-._.-._.-._.-._.-._.-._.-._');
    return next();
  }
  ...
  //-----------------------------------------------------------------------------
  // Checks Cyrillux app specific query parameters
  //  - possible Cyrillux query parameters are: 
  //  - not all parameters are present in every request
  //
  function checkCyrilluxQueryParams(queryParams) {
    return [];
  }
